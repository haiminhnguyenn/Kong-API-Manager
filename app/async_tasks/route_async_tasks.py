...
def create_kong_gw_route(data):
    url = f"{app.config['KONG_ADMIN_URL']}/routes"
    try:
...
@celery.task
def create_kong_gw_route_for_service(service_identifier, data):
...
def update_kong_gw_route(route_identifier, data):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if route_to_update.plugins:
                plugins_for_updated_route = db.session.query(PluginConfiguration).filter(
                    PluginConfiguration.route_id == route_to_update.id
                ).all()
                for plugin in plugins_for_updated_route:
                    plugin.route.refresh_updated_at(route_to_update.updated_at)
                    for key, value in data.items():
                        setattr(plugin.route, key, value)
            
            if route_to_update.service_id is not None:
...
def delete_kong_gw_route(route_identifier):
    ...
    try:
        ...
        if response.status_code == 204:
            ...
            if route_to_delete.service_id is not None:
